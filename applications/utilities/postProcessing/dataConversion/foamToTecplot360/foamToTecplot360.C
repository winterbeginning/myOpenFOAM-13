...
)
{
    ...
    forAll(patches, patchi)
    {
        ...
        {
            Info << "    excluding patch " << patchi << " " << pp.name()
                 << endl;
        }
        else
        {
...
{
    argList::addNote("Tecplot binary file format writer");
...
#include "createTime.H"
    ...
    {
        WarningInFunction
            << "Using neighbouring cell value instead of patch value" << nl
            << endl;
    }

    if (noPointValues)
    {
        WarningInFunction << "Outputting cell values only" << nl << endl;
    }
...
#include "createRegionMeshNoChangers.H"
    ...
    if (isDir(fvPath))
    {
        if (args.optionFound("time") || args.optionFound("latestTime") ||
            cellSetName.size() || regionName != polyMesh::defaultRegion)
        {
    ...
    vtkMesh vMesh(mesh, vtkTopo::vtkPolyhedra::none, cellSetName);
    ...
    forAll(timeDirs, timeI)
    {
        ...
        tecplotWriter::getTecplotNames(
            vsf, ValueLocation_CellCentered, varNames, varLocation);
        tecplotWriter::getTecplotNames(
            vsf, ValueLocation_CellCentered, cellVarNames, cellVarLocation);

        tecplotWriter::getTecplotNames(
            vvf, ValueLocation_CellCentered, varNames, varLocation);
        tecplotWriter::getTecplotNames(
            vvf, ValueLocation_CellCentered, cellVarNames, cellVarLocation);

        tecplotWriter::getTecplotNames(
            vSpheretf, ValueLocation_CellCentered, varNames, varLocation);
        tecplotWriter::getTecplotNames(vSpheretf,
                                       ValueLocation_CellCentered,
                                       cellVarNames,
                                       cellVarLocation);

        tecplotWriter::getTecplotNames(
            vSymmtf, ValueLocation_CellCentered, varNames, varLocation);
        tecplotWriter::getTecplotNames(
            vSymmtf, ValueLocation_CellCentered, cellVarNames, cellVarLocation);

        tecplotWriter::getTecplotNames(
            vtf, ValueLocation_CellCentered, varNames, varLocation);
        tecplotWriter::getTecplotNames(
            vtf, ValueLocation_CellCentered, cellVarNames, cellVarLocation);

        // pointFields
        tecplotWriter::getTecplotNames(
            psf, ValueLocation_Nodal, varNames, varLocation);
        ...
        if (args.optionFound("faceSet"))
        {
            ...
            fileName patchFileName(fvPath / setName / setName + "_" + timeDesc +
                                   ".plt");
            ...
            const indirectPrimitivePatch ipp(
                IndirectList<face>(mesh.faces(), faceLabels), mesh.points());

            writer.writePolygonalZone(setName, strandID++, ipp, allVarLocation);
            ...
            forAll(vsf, i)
            {
                writer.writeField(writer.getFaceField(
                    linearInterpolate(vsf[i])(), faceLabels)());
            }
            forAll(vvf, i)
            {
                writer.writeField(writer.getFaceField(
                    linearInterpolate(vvf[i])(), faceLabels)());
            }
            forAll(vSpheretf, i)
            {
                writer.writeField(writer.getFaceField(
                    linearInterpolate(vSpheretf[i])(), faceLabels)());
            }
            forAll(vSymmtf, i)
            {
                writer.writeField(writer.getFaceField(
                    linearInterpolate(vSymmtf[i])(), faceLabels)());
            }
            forAll(vtf, i)
            {
                writer.writeField(writer.getFaceField(
                    linearInterpolate(vtf[i])(), faceLabels)());
            }
            writer.writeConnectivity(ipp);
        ...
        if (doFaceZones && zones.size() > 0)
        {
            ...
            if (vMesh.useSubMesh())
            {
                patchFileName = fvPath / "faceZones" / cellSetName + "_" +
                                timeDesc + ".plt";
            }
            else
            {
                patchFileName = fvPath / "faceZones" / "faceZones" + "_" +
                                timeDesc + ".plt";
            }
            ...
            forAll(zones, zoneI)
            {
                ...
                {
                    const indirectPrimitivePatch ipp(
                        IndirectList<face>(mesh.faces(), pp), mesh.points());
                    ...
                    {
                        writer.writeField(writer.getFaceField(
                            linearInterpolate(vsf[i])(), pp)());
                    }
                    forAll(vvf, i)
                    {
                        writer.writeField(writer.getFaceField(
                            linearInterpolate(vvf[i])(), pp)());
                    }
                    forAll(vSpheretf, i)
                    {
                        writer.writeField(writer.getFaceField(
                            linearInterpolate(vSpheretf[i])(), pp)());
                    }
                    forAll(vSymmtf, i)
                    {
                        writer.writeField(writer.getFaceField(
                            linearInterpolate(vSymmtf[i])(), pp)());
                    }
                    forAll(vtf, i)
                    {
                        writer.writeField(writer.getFaceField(
                            linearInterpolate(vtf[i])(), pp)());
                    }
        ...
        fileNameList cloudDirs(readDir(runTime.timePath() / regionPrefix /
                                           lagrangian::cloud::prefix,
                                       fileType::directory));

        forAll(cloudDirs, cloudI)
        {
            IOobjectList sprayObjs(mesh,
                                   runTime.name(),
                                   lagrangian::cloud::prefix /
                                       cloudDirs[cloudI]);
            ...
            if (positionsPtr)
            {
                ...
                fileName lagrFileName(fvPath / lagrangian::cloud::prefix /
                                          cloudDirs[cloudI] /
                                          cloudDirs[cloudI] +
                                      "_" + timeDesc + ".plt");
                ...
                {
                    positions[n++] = elmnt().position(mesh);
                }
                ...
                tecplotWriter::getTecplotNames<label>(labelNames,
                                                      ValueLocation_Nodal,
                                                      allVarNames,
                                                      allVarLocation);

                tecplotWriter::getTecplotNames<scalar>(scalarNames,
                                                       ValueLocation_Nodal,
                                                       allVarNames,
                                                       allVarLocation);

                tecplotWriter::getTecplotNames<vector>(vectorNames,
                                                       ValueLocation_Nodal,
                                                       allVarNames,
                                                       allVarLocation);
                ...
                writer.writeOrderedZone(cloudDirs[cloudI],
                                        strandID++, // strandID,
                                        parcels.size(),
                                        allVarLocation);
